<script>
	...
	import Newone from "./init/Newone.svelte";
	...
	let count = 0;
	let steps = ["cardboard", "corpboard", "newspaper", "transaction", "roundclear", "roundstart", "newone"];
...
<div class="wrapper" transition:fade>
	{#if step === "cardboard"}
		<CardBoard bind:done={stepDone} />
	{:else if step === "corpboard"}
		<CorpBoard bind:done={stepDone} />
	...
	{:else if step === "roundclear"}
		<RoundClear bind:count />
	{:else if step === "roundstart"}
		<RoundStart bind:done={stepDone} />
	{:else if step === "newone"}
		<Newone bind:done={stepDone} />
	{/if}
	{#if diagOpen}
		<Typer bind:dialog={diag} bind:isopen={diagOpen} />
	{/if}
	<Taskbar bind:next={stepDone} bind:count {steps} />
</div>
